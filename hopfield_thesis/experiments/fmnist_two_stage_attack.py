"""
Fashion-MNIST two-stage one-pixel attack experiment.
...
Applies the same two-stage pipeline as grayscale_cifar_two_stage_attack:
  1. Precompute adversarial images with the inline attacker (cosine-delta + first-order
     sensitivity fallback) and save them to one_pixel_test/fmnist_seed_k/.
...
Run: python -m experiments.fmnist_two_stage_attack
"""
...
def load_fmnist() -> tuple[torch.Tensor, torch.Tensor]:
    ds = torchvision.datasets.FashionMNIST(root=str(DATA_DIR), train=True, download=True)
...
def run_experiment(
    ...
    for seed in SEEDS:
        seed_dir = ONE_PIX_DIR / f"fmnist_seed_{seed}"
        seed_dir.mkdir(exist_ok=True)

        X_gray, _   = sample_class_balanced((images, labels), N, seed=seed)
        stored      = X_gray.T.contiguous()
        ...
        attacks: list[dict] = []
        for i in range(N):
            cls_name = FMNIST_CLASSES[i // (N // len(FMNIST_CLASSES))].replace("/", "-")
        ...
        for i in range(N):
            ...
            cls_name = FMNIST_CLASSES[i // (N // len(FMNIST_CLASSES))]
            detail.append({
...
def save_figure(
    ...
    class_names: list[str] = FMNIST_CLASSES,
) -> None:
...
def build_report(detail: list[dict], summary: list[dict], dataset_name: str) -> str:
    ...
    p()
    p(f"Images saved to: one_pixel_test/{dataset_name.lower().replace(' ', '_').replace('-', '_')}_seed_k/")
...
def main() -> None:
    ...
    name = "fmnist"
    print("=" * 64)
    print("Fashion-MNIST Two-Stage One-Pixel Attack Experiment")
    print("=" * 64)

    print("\nLoading Fashion-MNIST ...")
    images, labels = load_fmnist()
    ...
    report = build_report(detail, summary, "Fashion-MNIST")
    print()
    print(report)
    ...
    save_figure(
        ...
        FIG_DIR / "fmnist_attack_grid.png",
